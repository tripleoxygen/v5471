...
var Page = (function($, App) {
    ...
    vRule = {
        "ruleName": [
            {type: "required", message: 'Please provide a rule name (alpha-numeric).'},
            {type: "pattern", pattern: /^[0-9A-Za-z_-]+$/, message: 'Rule name must be alpha numeric.'}
        ],
        "localPort": [
            {type: "required", message: 'Please provide port(s) or port ranges.'},
            {type: portValidator,message: 'Please provide a valid port range i.e. 1:65535.'}
        ],
        "remotePort": [
            {type: "required", message: 'Please provide port(s) or port ranges.'},
            {type: portValidator,message: 'Please provide a valid port range i.e. 1:65535.'}
        ...
        "localIP": [
            { type: "required",message: 'Please provide a destination IP address.'},
            { type: "ipAddress4and6",message: 'Please provide a valid IP address.'}
        ],
        "remoteIP": [
            { type: "required",message: 'Please provide a destination IP address.'},
            { type: "ipAddress4and6",message: 'Please provide a valid IP address.'}
    ...
    function changeDefaultPolicy(value){
        ...
        cli.commit(function(res) {
            if(!res.error) {
                Messages.info('Default policy successfully changed.');
                if (value == 'Accept') {
    ...
    function removeRule(index){
        if (isEdit) {
            alert('Please save or cancel edited rule before');
            return;
        }
        Messages.clearAll();
        cli.rollback();
        ...
        cli.commit(function(res) {
            if(!res.error) {
                Messages.info('Firewall rule successfully removed.');
        ...
        for (var i=0 ; i<8; i++) {
            switch(i) {
                ...
                case 4 : trBody += '<td><select  id="actionEdit" style="font-size:10px"  oldvalue="'+cells[i].firstChild.getAttribute('src')+'">\
                                                        <option value="acptLocal" '+ (cells[i].innerHTML.indexOf('acptLocal') != -1 ? "selected":"")+'>Accept from local</option>\
                                                        <option value="acptRemote" '+ (cells[i].innerHTML.indexOf('acptRemote') != -1 ? "selected":"")+'>Accept from remote</option>\
                                                        <option value="rjctLocal" '+ (cells[i].innerHTML.indexOf('rjctLocal') != -1 ? "selected":"")+'>Reject from local</option>\
                                                        <option value="rjctRemote" '+ (cells[i].innerHTML.indexOf('rjctRemote') != -1 ? "selected":"")+'>Reject from remote</option>\
                                                    </select></td>';
                            break;
        ...
        if(ruleValidatorLine.validate()  && checkIPsFormat($("#remoteIPEdit").val(), $("#localIPEdit").val())) {
            ...
            cli.commit(function(res) {
                if(res.error) {
                    ...
                    Messages.info('Rule successfully updated.');
                    clearRule();
                    var trBody ='';
                    var tr = document.getElementById('rules'+index);
                    var cells = tr.getElementsByTagName('td');
    ...
    function populateRulesList(){
        ...
        tableBody +=  '<tr><td>Default</td><td>tcp,udp</td><td>*</td><td>*</td><td id="defaultimg"><img src="../../img/'+defaultAction+'.png"></td><td>*</td><td>*</td><td></td></tr>';
            ...
            if(ruleValidator.validate() && checkIPsFormat($("#remoteIP").val(), $("#localIP").val())) {
                ...
                cli.commit(function(res) {
                    if(res.error) {
                        ...
                        Messages.info('New rule successfully added.');
