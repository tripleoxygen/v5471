...
var PerfDiag = (function ($) {
    ...
    var float_type = {
        type: "pattern",
        pattern: /^([+-]{1})*[0-9]+([\.[0-9]+)*$/,
        message: 'You must type a float for this field.'
            };
    var int_type = {
        type: "number",
        message: 'You must type an integer for this field.'
    };
    var vRules = {
        PerfDiag: {
            MaxPer: [float_type],
    ...
    function loadMonitor() {
        ...
        if (hpna.CurrentInstance.Status  == "2") {
            ...
            $("#hpna_status strong").html('HPNA - Ready');
        } else {
            $("#hpna_status").removeClass("hpna_status_on");
            $("#hpna_status strong").html('HPNA - Not Ready');
        }
    ...
    function commit_error(xhr, code, message) {
        ...
        var info = warnMsg('Settings saved errors.');
        console.log(message);
    }
    ...
    function commit_succeed(res) {
        ...
        if (res.error != undefined) {
            ...
            infoMsg('Settings saved succeed.');
        }
        return true;
    }
    ...
    function netinfStationToHtml(station) {
        ...
        var res = (station.Success == undefined && station.TestStatus == "1") ? 'PASS' : 'FAILED';
        var res_css = (station.Success == undefined && station.TestStatus == "1") ? "success" : "failed";
        html_data += '<tr><td class="cinza text-left">Result</td><td class="cinza text-left ' + res_css + '">' + res + '</td></tr>';
    ...
    function dpy_netinf_sum() {
        ...
        $("#netinf_results").html((succeed == 1 ) ? 'TEST PASS' : 'TEST FAILED');
        ...
        for (x = 0; x < sum_tab.length; x++) {
            ...
            for (y = 0; y < sum_tab.length; y++) {
                switch (sum_tab[x][y]) {
                    case 1:
                    html_sum_tab += "<td class=\"success\">" + 'PASS' + "</td>";
                    // test succeed
                        break;
                    case 0:
                    html_sum_tab += "<td class=\"failed\">" + 'FAILED' + "</td>";
                    // test failed
                        break;
                    case -1:
            ...
            switch(res.HPNA_PerfDiag.DiagState) {
                ...
                case "done":
                    ...
                    if (res.HPNA_PerfDiag.Station.Count > 0) {
                        ...
                        var data_idx = $("#station_list").elements[0].options[idx].value;
                        fillStationResult(data_idx);
                        infoMsg('<strong >Synced: All Datas received.</strong>');
                    }
                    
                break;
                case "error":
                    $(".busy").removeClass("show");
                    errMsg('Error while syncing hpna stations. Be sure  gw is connected to at least one other device using HPNA connector');
                break;
            };
        }
    }
        ...
        if (res != undefined) {
            ...
            switch(res.HPNA_NetPerDiag.DiagState) {
                ...
                case "done":
                    ...
                    infoMsg('<strong >Monitor: All Datas received.</strong>');
                break;
                case "error":
                    $(".busy").removeClass("show");
                    errMsg('Error while syncing hpna stations. <br/>Be sure  gw is connected to at least one other device using HPNA connector');
                break;
            };
        }
    }
    ...
    function display_netinf_popup(source) {
       ...
       doc.body.innerHTML = "";
       doc.write('<html><head><title>Network Test (NETINF)</title>');
       doc.write('<link rel="stylesheet" href="../../css/gateway.css" />');
       doc.write('<body class="popup"> <center>');
       doc.write('<H1>Network Test (NETINF)</H1>');
    ...
    function display_netper_popup(source) {
       ...
       popup = window.open('', source, 'height=500,width=700');
       var doc = popup.document;
       doc.write('<html><head><title>Monitor (NETPER)</title>');
       doc.write('<link rel="stylesheet" href="../../css/gateway.css" />');
       doc.write('<body class="popup">');
       doc.write('<H1>Monitor (NETPER)</H1>');
    ...
    var netinf_but_action = {
        ...
        refresh: function () {
            ...
            if (!validator.perfDiag.validateAll()) {
                warnMsg('Please, validate the conf first');
                return false;
            }
        ...
        cmd: function () {
            ...
            if (hpna.CurrentInstance.Status != 2 || hpna.StationDiag.Station.Count < 2) {
                errMsg('Be sure gw is connected to at least one other device using HPNA connector.' + "<br/>" + 'Click on REFRESH to update neighbors station list.');
                return;
            }
            cli.rollback();
        ...
        save_results: function () {
            if (netinf_test_done == netinf_state_enum.COMPUTING) {
                infoMsg('Warning: The test is computing. Please wait !!!');
                return;
            } else if (netinf_test_done == netinf_state_enum.NOTDONE) {
                infoMsg('Warning: You must execute a test before saving results.');
                return;
            }
            dpy_netinf_sum();
            display_netinf_popup("Netinf");
