var WirelessUtil = (function() {
    return {
        ...
        hexa2Ascii: function(inHexa) {
            ...
            if ((inHexaLen % 2) != 0) {
                throw ('A sequência hexadecimal esta incompleta');
            }
            for (var i = 0; i < inHexaLen; i++) {
                tmpByte = inHexa.charAt(i);
                if (((tmpByte < '0') || (tmpByte > '9')) && ((tmpByte < 'A') || (tmpByte > 'F'))) {
                    throw 'A sequência hexadecimal contém caracteres não-hexadecimais.';
                }
                if ((i % 2) == 0) outAscii += '%';
                outAscii += tmpByte;
            }
            ...
            if (outAsciiLen != Math.floor(inHexaLen / 2)) {
                throw 'A sequência hexadecimal contém caracteres não imprimíveis.';
            }
...
var Page = (function($, App) {
    ...
    vRules = {
        "WLANInterface_1_Config_SSID": [
            {
                type: "required",
                message: 'Por favor informe um nome de rede (SSID).'
            },
            {
                type: "pattern",
                pattern: /^[0-9A-Za-z%\(\?>@\)\^~"_=\+'&\.\]/\\| \*<!#\$\-\[\{\}:;,]+$/,
                message: 'O nome de rede precisa ser alfanumérico.'
            }
        ],
        "WLANInterface_1_Config_WPSPINCode": {
            ...
            message: 'Por favor informe um PIN.'
        },
        "wirelessPassword": {
		...
		if(!pass && sec != 'BasicNone') {
                    opts.message = 'Por favor informe uma senha.';
                    return false;
                }
                
                // check for spaces
                if(pass.indexOf('\\s') != -1) {
                    opts.message = 'Senhas não podem conter espaços em branco';
                }
                ...
                if(sec === "WPA" || sec === "WPA2" || sec === "WPA-Auto") {
                    if(! WirelessUtil.isHexadecimal(pass, false, 64)) {
                        if(passLen < 8 || passLen > 63 || !WirelessUtil.isAscii(pass, false, passLen)) {
                            opts.message = 'A senha precisa ter 64 dígitos hexadecimais ou de 8 a 63 caracteres ASCII.';
                            return false;
                        }
                    }
                }
                ...
                if(sec === "Basic") {
                    if(! WirelessUtil.isHexadecimal(pass, false, 10) && !WirelessUtil.isHexadecimal(pass, false, 26)) {
                        if(pass.length !== 5 && pass.length !== 13) {
                            opts.message = 'A senha precisa ser  (10 ou 26) caracteres hexadecimais ou (5 ou 13) ASCII.';
                            return false;
                        }
                    }
                }
    ...
    function showWpsSuccess(res) {
        if(res.error) {
            var method = $("#WLANInterface_1_WPSMethod").val();
            if(method === "PIN") {
                Messages.error('PIN inválido. Por favor tente novamente.');
            }else {
                Messages.error('Erro ao iniciar conexão WPS.');
            }
        }else {
            Messages.info('Conexão WPS iniciada...');
        }
    }
    ...
    function updateWLanIfData(wlanid) {
        ...
        cli.rollback().read(key)
            .commit(function(res) {
                if(res.error) {
                    Messages.error('Erro ao adquirir dados do Wi-Fi. Por favor atualize a página.');
                }else {
                    data.WLANInterface["" + wlanid] = res[key];
                    populateBasForm();
                    populateAdvForm();
                    Messages.info('Configuração do Wi-Fi salva com sucesso.');
                }
            });
    }
    ...
    function populateBasForm() {
        ...
        if( standard == "11n" || standard == "11gn" || standard == "11bgn") {
            $("#WLANInterface_1_Config_BeaconType").append(['<option value="WPA-Auto">WPA/WPA2</option>',
                                                            ...
                                                            '<option value="BasicNone">Nenhum (Não recomendado)</option>']);
        }
        else {
            $("#WLANInterface_1_Config_BeaconType").append(['<option value="WPA-Auto">WPA/WPA2</option>',
                                                          '<option value="WPA2">WPA2</option>',
        ...
        $("#addFiltering").bind("click", function() {
            ...
            if(filteringValidator.validate()) {
                ...
                if (index = addMacFiltering(datas["Hostname"],datas["MACAddress"]) > 0) {
                    Messages.info('Regra de Filtro de MAC adicionada com sucesso.');
                    ...
                    Messages.error('Este Endereço MAC já existe.');
                }
            }
        });
    }
    ...
    function setupUi() {
        ...
        $(".switchButton").bind("click", function() {
            ...
            if(this.id == "WLANInterface_1_Config_HideSSID" && id.attr("state") ==  1) {
                if (! confirm('Isto também irá desabilitar a função WPS, você deseja continuar? ')) { 
                    return;
                } else {
