var Page = (function($, App) {
    ...
    function showLansStatus() {
        ...
        for (var j = 1; j <= data.lanDevices.Count; j++){
            if (data.lanDevices[j].InterfaceType == "Ethernet") {
                html += 'NOME DO DISPOSITIVO: '+data.lanDevices[j].Hostname+'\n';
    ...
    function showSTBStatus() {
        ...
        if (data.ledTvState.List != undefined && data.ledTvState.List != "") {
            ...
            if (data.ledTvState[idx].State == "On") {
                ...
                STBDetail.find("span.state").html('Disponível');
                } 
            }
        ...
        $("#hpna").html(data.hpna == 1 ? 'Habilitado' : 'Desabilitado' );
    ...
    function showWirelessStatus() {
        ...
        if(wlEnable) {
            ...
            switch (wLanIntf.Config.BeaconType) {
                ...
                break;
                case "Basic":
                $("#beaconType").html((wLanIntf.Config.WEPEncryption) == "None" ? 'Nenhum' : 'WEP');
                break;
                default :  $("#beaconType").html(wLanIntf.Config.BeaconType);
                break;
            }
            if (wLanIntf.WPSEnable == 1) {
                $("#WPSEnable").html('Habilitado');
            } else {
                $("#WPSEnable").html('Apagado:');
            }
            if (wLanIntf.Config.HideSSID == 1) {
                $("#HideSSID").html('Não');
            } else {
                $("#HideSSID").html('Sim');
                $("#wirelessVisibility").html('Visível ');
            }
            if (wlanConf.Channel == 0) {
                $("#Channel").html('Automático');
            } else {
                $("#Channel").html(wlanConf.Channel);
            }
            if (wLanIntf.Station.Count >0 ) {
                $("#wifiStation").html(wLanIntf.Station.Count +' Dispositivo(s) Wi-Fi Conectado(s):\n');
            }
    ...
    function showInternetStatus() {
        ...
        switch(netInfo.wan_cnx_type) {
            case cnx_type.xDSL:
                ...
                if(netInfo.linkUp) {
                    ...
                    dslDetails.find("span.state").html('Sincronizado')
                }
                break;
            case cnx_type.eth:
                ...
                if(netInfo.linkUp) {
                    ...
                    ethDetails.find("span.state").html('Conectado');
                }
                break;
            default:
                break;
        ...
        if (netInfo.linkUp && activeWanDevice.Status.State === "Up") {
            ...
            pppDetails.find("span.state").html('Conectado');
        }
    }
    ...
    function initUI() {
        ...
        $(".status-link a").bind("click", function() {
            if ( $("#"+$(this).attr('id')+"-more").getStyle("display") == 'none' ) {
                ...
                $("#"+$(this).attr('id')).html('<u>Ocultar</u>');
            } else {
                $("#"+$(this).attr('id')+"-more").css("display", "none");
                $("#"+$(this).attr('id')).html('<u>Mais detalhes</u>');
            }
        });
